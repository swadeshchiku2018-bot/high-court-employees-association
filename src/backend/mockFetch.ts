...
// In‑memory store for admin blog posts under Key Initiatives
let keyInitiatives: { id: string; title: string; content: string; author: string; createdAt: string }[] = [];
...
export const initMockFetch = () => {
  window.fetch = async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    try {
      ...
      if (path === '/api/office-bearers' && method === 'POST') return jsonRes(backendStore.updateOfficeBearer(body.bearer || body, body.actorName));
      if (path.match(/^\/api\/office-bearers\/([^\/]+)$/) && method === 'PUT') return jsonRes(backendStore.updateOfficeBearer({ ...(body.bearer || body), id: path.split('/')[3] }, body.actorName));
      if (path.match(/^\/api\/office-bearers\/([^\/]+)$/) && method === 'DELETE') return jsonRes(backendStore.deleteOfficeBearer(path.split('/')[3], body?.actorName));
      ...
      if (path.match(/^\/api\/notices\/([^\/]+)$/) && method === 'GET') {
        const notice = backendStore.getNotices().find(n => n.id === path.split('/')[3]);
        return notice ? jsonRes(notice) : new Response(JSON.stringify({ error: "Notice not found" }), { status: 404 });
      }
      ...
      // 14. Key Initiatives
      if (path === '/api/key-initiatives' && method === 'GET') return jsonRes(keyInitiatives);
      if (path === '/api/key-initiatives' && method === 'POST') {
        const newItem = {
          id: `ki-${Date.now()}`,
          title: body.title,
          summary: body.summary || '',
          content: body.content,
          tag: body.tag || 'Welfare',
          author: 'Admin',
          createdAt: new Date().toISOString()
        };
        keyInitiatives.unshift(newItem);
        return jsonRes(newItem, 201);
      }
      if (path.match(/^\/api\/key-initiatives\/([^\/]+)$/) && method === 'DELETE') {
        const id = path.split('/')[3];
        keyInitiatives = keyInitiatives.filter(k => k.id !== id);
        return jsonRes({ success: true });
      }
