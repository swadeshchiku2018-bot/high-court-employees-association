...
class BackendStore {
  ...
  deleteOfficeBearer(id: string, actorName: string = "Admin") {
    const idx = this.officeBearers.findIndex(ob => ob.id === id);
    if (idx >= 0) {
      const removed = this.officeBearers[idx];
      this.officeBearers.splice(idx, 1);
      this.addAuditLog(actorName, "PRESIDENT", "OFFICE_BEARER_DELETED", `Removed office bearer: ${removed.name} (${removed.designation})`);
    }
    return this.getOfficeBearers();
  }
  ...
  registerNewMember(data: Omit<Member, 'id' | 'membershipId' | 'status' | 'monthlyContribution'>): Member {
    ...
    const newMember: Member = {
      ...data,
      id: `mem-${Date.now()}`,
      membershipId: `OHCEA-2026-${nextNum}`,
      status: 'PENDING',
  ...
  updateMemberStatus(id: string, status: Member['status'], actorName: string = "President", notes?: string): Member {
    ...
    if (status === 'ACTIVE') {
      ...
      this.addNotification(member.id, "Membership Approved", `Congratulations! Your OHCEA Membership (${member.membershipId}) has been verified and activated. You can now download your digital ID card.`, "APPROVAL");
    } else if (status === 'REJECTED') {
      member.rejectionReason = notes;
      this.addNotification(member.id, "Membership Application Update", `Your membership application requires clarification: ${notes || 'Please check with OHCEA Secretariat.'}`, "APPROVAL");
    }
