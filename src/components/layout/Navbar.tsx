...
import { Shield, User, LogOut, Bell, LayoutDashboard, IdCard as IdCardIcon, ChevronDown, Menu, X } from 'lucide-react';
...
export const Navbar: React.FC<NavbarProps> = ({ activeTab = 'home', onNavigate }) => {
  ...
  const navLinks = [
    ...
    { id: 'welfare', label: 'Key Initiatives', path: '/key-initiatives' },
  ...
  const handleFontSize = (size: 'sm' | 'md' | 'lg') => {
    ...
    if (size === 'lg') document.documentElement.style.fontSize = '110%';
  };
      ...
      <div className="bg-[#f5f5f5] border-b border-slate-300 py-1.5 px-4 text-xs text-slate-700">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <button onClick={handleSkipToMain} className="hidden sm:block hover:underline cursor-pointer">Skip to Main Content</button>
            <div className="flex items-center gap-1">
              <button onClick={() => handleFontSize('sm')} className="w-6 h-6 bg-white border border-slate-300 flex items-center justify-center hover:bg-slate-100 cursor-pointer text-[10px]" title="Decrease font size">A-</button>
              <button onClick={() => handleFontSize('md')} className="w-6 h-6 bg-white border border-slate-300 flex items-center justify-center hover:bg-slate-100 cursor-pointer text-xs" title="Reset font size">A</button>
              <button onClick={() => handleFontSize('lg')} className="w-6 h-6 bg-white border border-slate-300 flex items-center justify-center hover:bg-slate-100 cursor-pointer text-sm" title="Increase font size">A+</button>
            </div>
          </div>
        </div>
      </div>
        ...
        >
          ...
          <div className="border-l-[2.5px] border-amber-500 pl-4 py-1.5">
            <h1 className="font-bold text-[22px] md:text-2xl text-[#002855] leading-tight uppercase font-serif tracking-wide">
              Orissa High Court Employees' Association
            </h1>
