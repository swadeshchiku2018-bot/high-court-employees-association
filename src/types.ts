...
export interface Member {
  ...
  email: string;
  role: UserRole;
  membershipId: string; // e.g. OHCEA-2026-0001
  employeeCode: string;
  name: string;
  avatarUrl: string;
  designation: string;
...
export type WelfareGrant = WelfareApplication;
...
export interface FundTransaction {
  ...
  transactionId: string;
  type: 'CREDIT' | 'DEBIT';
  category: 'CONTRIBUTION' | 'WELFARE_DISBURSED' | 'EVENT_EXPENSE' | 'ADMIN_EXPENSE' | 'DONATION' | 'INTEREST' | 'SPECIAL_GRANT' | 'CORPUS_ALLOCATION';
  description: string;
  amount: number;
  balanceAfter: number;
  referenceNo: string;
